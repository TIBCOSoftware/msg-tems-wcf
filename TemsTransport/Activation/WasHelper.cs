...
namespace com.tibco.wcf.tems.ActivatorService.Activation
{
    ...
    public class WasTemsInfo
    {
        public const string TemsScheme = "net.tems";
        public const string ServiceName = "NetTemsActivator";
    }
    class WasHelper
    {
        ...
        internal static TraceSource ts = new TraceSource("TemsActivationTrace");
        static int errseq = 0;

        internal static int GetErrSeq()
        {
            return Interlocked.Increment(ref errseq);
        }
        ...
        static Delegate GetProcDelegate<TDelegate>(SafeFreeLibrary lib, string procName)
        {
            ...
            {
                throw new Win32Exception(string.Format("GetProcDelegate: Function '{0}' not found.", procName));
            }
    ...
    delegate void DataReceivedCallback(string wcfDestination);
}
